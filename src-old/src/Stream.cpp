...
void Stream::InitializePipewire() {

  m_Data.core = pw_context_connect_fd(m_Data.context, m_PWFD, nullptr, 0);

  if (m_Data.core == nullptr)
    throw std::runtime_error("Failed to connect to PipeWire remote");

  m_Data.stream = pw_stream_new(
      m_Data.core, "ssrd",
  ...
  pw_stream_add_listener(m_Data.stream, &m_Data.stream_listener, &stream_events,
                         this);
  ...
  pw_stream_connect(m_Data.stream, PW_DIRECTION_INPUT, m_TargetId,
                    (pw_stream_flags)(PW_STREAM_FLAG_AUTOCONNECT |
                                      PW_STREAM_FLAG_MAP_BUFFERS),
                    params, 1);

  m_Source = (PipeWireSource *)g_source_new(&pipewire_source_funcs,
                                            sizeof(PipeWireSource));

  m_Source->loop = m_Data.loop;

  g_source_add_unix_fd(&m_Source->base, pw_loop_get_fd(m_Data.loop),
                       (GIOCondition)(G_IO_IN | G_IO_ERR | G_IO_HUP));
  g_source_attach(&m_Source->base, NULL);
  g_source_unref(&m_Source->base);

  pw_loop_enter(m_Source->loop);
  m_IsPipewireInitialized = true;
}
...
void Stream::onStartSession(GObject *source_object, GAsyncResult *res,
  ...
  g_variant_get(child, "(u@a{sv})", &self->m_TargetId, nullptr);
  g_variant_unref(child);
  g_variant_unref(streams);

  self->m_PWFD = xdp_session_open_pipewire_remote(self->m_Session);
  if (self->m_PWFD < 0) {
    std::cerr << "Failed to open PipeWire remote" << std::endl;
    return;
  }

  self->m_IsSessionStarted = true;
}
...
void Stream::begin(std::function<void(spa_buffer *spa_buf)> onStream) {
  ...
  GMainContext *context = g_main_loop_get_context(m_MainLoop);
