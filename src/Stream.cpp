...
void Stream::onStartSession(GObject *source_object, GAsyncResult *res,
  ...
  self->m_Source = (PipeWireSource *)g_source_new(&pipewire_source_funcs,
                                                  sizeof(PipeWireSource));

  self->m_Source->loop = self->m_Data.loop;

  g_source_add_unix_fd(&self->m_Source->base, pw_loop_get_fd(self->m_Data.loop),
                       (GIOCondition)(G_IO_IN | G_IO_ERR | G_IO_HUP));
  g_source_attach(&self->m_Source->base, NULL);
  g_source_unref(&self->m_Source->base);
}
...
Stream::~Stream() {
  ...
  if (m_Source)
    g_source_destroy(&m_Source->base);
...
void Stream::begin(std::function<void(spa_buffer *spa_buf)> onStream) {
  ...
  //   if (m_Data.loop)
  //     pw_loop_iterate(m_Data.loop, true);
  // }
}